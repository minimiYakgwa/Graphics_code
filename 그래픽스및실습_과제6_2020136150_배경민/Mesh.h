...
class Mesh {			// Mesh 클래스
	int		nVtx;		// Vertex 개수
	Vertex*	vertex;		// Vertex 리스트
	int		nFace;		// Face 개수
	Face*	face;		// Face 리스트
	Normal*	normal;		// Normal 리스트
...
public:
	...
	void readAse(const char* fileName) { // ASE file load
		...
		while (fgets(line, 256, fp) != NULL) {
			sscanf(line, "%s", str);
			if (strcmp(str, "*MESH") == 0) {	//“*MESH”라는 문자가 나올 때까지 계속 읽음
				...
				if (strcmp(str, "*MESH_VERTEX_LIST") == 0) {
					for (int i = 0; i < nVtx; i++) {          // vertex 의 x, y, z 데이터 저장 
				...
				if (strcmp(str, "*MESH_FACE_LIST") == 0) {
					for (int i = 0; i < nFace; i++) {	// face 의 vertex 인덱스 데이터 저장
						fgets(line, 256, fp);	// Read the Line  *MESH_FACE_LIST   
				...
				fgets(line, 256, fp);  // Read the Line '}'
				fgets(line, 256, fp);  // Read the Line  *MESH_NUMCVERTEX ---> 없애야 함.
				sscanf(line, "%s", str);
				if (strcmp(str, "*MESH_NUMCVERTEX") == 0) { 	// 내용이 *MESH_NUMCVERTEX 이면 없앰.
					fgets(line, 256, fp);		// 다시  *MESH_NORMALS 를 위한 라인을 읽음
					sscanf(line, "%s", str);
				}
				if (strcmp(str, "*MESH_NORMALS") == 0) {    // normal 의 법선벡터 데이터 저장
	...
	void draw(float r, float g, float b, float scale = 1.0f, bool bCoord = false) {
		setColor(r, g, b, 1.0);	// 재질의 색을 설정한 후
		draw(scale, bCoord);
	}
